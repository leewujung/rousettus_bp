...
%             beampattern
% 2017 10 04  Update to include new piston model results (Monte Carlo
%             simulation using ectr beam center instead of max beam center)
...
piston_path = 'model_piston_proj_multifreq_20171003_std1.0';
...
% Mann-Whitney U-test
[mw.p_data_phase,mw.h_data_phase,mw.stat_data_phase] = ...
    ranksum(el_data(~data_idx_bad)./az_data(~data_idx_bad),...
            el_phase(~phase_idx_bad)./az_phase(~phase_idx_bad));
[mw.p_data_piston,mw.h_data_piston,mw.stat_data_piston] = ...
    ranksum(el_data(~data_idx_bad)./az_data(~data_idx_bad),...
            el_piston(~piston_idx_bad)./az_piston(~piston_idx_bad));
stat.ks = ks;
stat.mw = mw;
