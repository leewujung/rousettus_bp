...
simu_data_path = 'model_bp_proj_RaColony_20161012_std1.0';
...
% Set save_fname
ss_save = strsplit(D.bp_model_file,'_');
model_freq_str = ss_save{6};
tongue_loc_str = strtok(strjoin({ss_save{end-2:end}},'_'),'.');

save_fname = sprintf('%s_%s_%s_%s',...
    script_name,noise_std,model_freq_str,tongue_loc_str);
...
axesm(D.map.map_projection);
contour(-BP.xq,BP.yq,BP.vq_norm,cvec,'fill','on');
...
gridm('gcolor',cgrey,'glinestyle','-');
framem('fedgecolor',cgrey);
contourm(interp_avg_left.elq_avg,interp_avg_left.azq_avg,interp_avg_left.vq_norm_avg,cvec,'fill','on');
...
gridm('gcolor',cgrey,'glinestyle','-');
framem('fedgecolor',cgrey);
contourm(interp_avg_right.elq_avg,interp_avg_right.azq_avg,interp_avg_right.vq_norm_avg,cvec,'fill','on');
