...
% Statistical tests
[h_data_phase,p_data_phase] = ...
    kstest2(el_data(~data_idx_bad)./az_data(~data_idx_bad),...
            el_phase(~phase_idx_bad)./az_phase(~phase_idx_bad),'alpha',0.01);

[h_data_piston,p_data_piston] = ...
    kstest2(el_data(~data_idx_bad)./az_data(~data_idx_bad),...
            el_piston(~piston_idx_bad)./az_piston(~piston_idx_bad),'alpha', ...
            0.01);


% Save results
save(fullfile(save_path,[save_fname,'_results.mat']),...
     'param','data*','piston*','phase*','h_*','p_*','el_*','az_*');
...
saveas(fig_ar,...
    fullfile(save_path,[save_fname,'_elaz.fig']),'fig');
saveSameSize_res(fig_ar,150,'file',fullfile(save_path,[save_fname,'_elaz.png']),...
    'format','png','renderer','painters');
