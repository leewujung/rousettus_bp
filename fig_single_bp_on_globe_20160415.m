...
% data_path = ['C:\Users\',usrn,'\Dropbox\0_ANALYSIS\bp_processing\proc_output'];
data_path = 'C:\Users\Wu-Jung Lee\Documents\fromDropbox\bp_processing\proc_output_rousettus_checked';
data_file = 'rousettus_20150825_36134_02_mic_data_bp_proc.mat';
freq_wanted = 45*1e3;
call_num = 19:22;
% call_num = 19;
interp_opt = 'rb_rbf';
save_root_path = 'C:\Users\Wu-Jung Lee\Documents\fromDropbox\bp_processing\analysis_results_figs';
[~,fname_pre,~] = fileparts(mfilename('fullpath'));
save_folder = 'single_click_bp_contour_20160418';
% save_path = 

if ~exist(fullfile(save_root_path,save_folder),'dir')
...
for iC=call_num
    ss_title = sprintf('bat %s, trial %s, click #%02d, freq %d kHz',ss{3},ss{4},iC,freq_wanted/1e3);
    ss_file = sprintf('bat%s_trial%s_click%02d_freq%dkHz',ss{3},ss{4},iC,freq_wanted/1e3);
...
%     contour_vec = -3:-3:(floor(vq_norm_min/3)-1)*3;
    ...
    axis off
    contourfm(elq/pi*180,azq/pi*180,vq_norm,contour_vec(2:cvec_min_idx),'w','linewidth',1);  % don't plot 0 dB contour
    hold on
    ...
    title(ss_title)
    set(gcf,'color','w');
    saveas(gcf,fullfile(save_root_path,[ss_file,'.png']),'png');
    saveas(gcf,fullfile(save_root_path,[ss_file,'.fig']),'fig');
    export_fig(fullfile(save_root_path,ss_file),'-q101','-eps');  % high quality EPS
