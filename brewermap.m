...
% brewermap('*PuOr'); % Set the default colorscheme.
% load topo
% load coast
% figure
...
if nargin==0
    ...
    assert(~isempty(dcs),'You need to save a default colorscheme first...')
    [map,num,typ] = cbSample([],dcs);
elseif nargin==2
    % Use the provided colormap length and the provided colorscheme.
    assert(isnumeric(N),'First argument must be a numeric.')
