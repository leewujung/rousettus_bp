...
%             Change from loading mean az or el beamwidth to directly
%             obtain from data files (this part taken from
%             'fig_azel_distr_indiv_composite_model_20170412.m'
...
rotate_data_path = 'rotate_all_click_2010308';  % 20170308
...
S.param.indiv_data_path = indiv_data_path;
S.param.indiv_data_file = indiv_data_file;
...
for iF=1:length(indiv_data_file)
    ...
    if ~isempty(B_data.rot_elpctr_tilt)
        ...
        e(iF,1) = B_data.rot_elpctr_tilt.E.e;
        a0(iF,1) = B_data.rot_elpctr_tilt.E.a0;
        b0(iF,1) = B_data.rot_elpctr_tilt.E.b0;
        theta(iF,1) = B_data.rot_elpctr_tilt.E.theta;
    else
        ar(iF,1) = NaN;
        elps_x_max(iF,1) = NaN;
        elps_y_max(iF,1) = NaN;
...
switch match
    case 'az'
        match_deg = nanmean(az_data);
    case 'el'
        match_deg = nanmean(el_data);
    case 'azel'
        match_deg = nanmean([az_data;el_data]);
end
