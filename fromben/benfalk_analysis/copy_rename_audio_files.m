  ...
  for bd=1:length(BD)
    ...
    for fd=1:length(FD)
      ...
      if ~exist(exportfn,'file')
        ...
        if strcmp(type_file,'mic_data_detect')
          datafname=[new_top_dir strjoin({species,datename,band,trialnum,'mic_data'},'_') '.mat'];
          if exist(datafname,'file')
            m=matfile(datafname);
            ch_num=size(m.sig,2);
            mm=matfile(exportfn,'writable',true);
            mm.num_ch_in_file=ch_num;
          end
        end
      end
    end
