%  side=determine_side(bat,pts,frms,useline,DIAG)
%
...
%   end
  ...
  for fr=1:length(frms)
    ...
    if f+1 <= length(bat) && isfinite(bat(f+1,1))
      side(f)= sign( (bat(f+1,1)-bat(f,1))*(pts(f,2)-bat(f,2)) -...
        (bat(f+1,2)-bat(f,2))*(pts(f,1)-bat(f,1)) ) ;
      if nargin> 4 && DIAG
        figure(2), clf;
        plot3(bat(f:f+1,1),bat(f:f+1,2),bat(f:f+1,3),'.-','color',[.5 .5 .5])
        text(bat(f:f+1,1),bat(f:f+1,2),bat(f:f+1,3),['1';'2']);
        hold on;
        plot3(pts(f,1),pts(f,2),pts(f,3),'.k');
        view(2);
        if side(f)==1
          clr='r';
        else
        ...
        plot3(pts(f,1),pts(f,2),pts(f,3),['o' clr]);
        drawnow;
        pause(.05)
      end
    end
...
if nargin>4 && DIAG
  ...
  figure(1), clf;
  plot3(bat(frms,1),bat(frms,2),bat(frms,3),'.','color',[.5 .5 .5]);
  hold on;
  plot3(pts(:,1),pts(:,2),pts(:,3),'.k');
  h1=plot3(pts(side==1,1),pts(side==1,2),pts(side==1,3),'or');
  h2=plot3(pts(side==-1,1),pts(side==-1,2),pts(side==-1,3),'ob');
  grid on;
  text(bat(frms(1),1),bat(frms(1),2),bat(frms(1),3),'start')
  text(bat(frms(end),1),bat(frms(end),2),bat(frms(end),3),'end')
  if ~isempty(h1) && ~isempty(h2)
    legend([h1 h2],'Side = 1', 'Side = -1')
  end
  if useline
    view(-90,90);
  else
