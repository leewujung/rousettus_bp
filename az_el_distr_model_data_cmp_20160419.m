...
model_path = 'model_piston_proj_20160419';
...
for iF=1:length(data_file_all)
    B_data = load(fullfile(base_path,save_root,data_path,data_file_all(iF).name));
    B_model = load(fullfile(base_path,save_root,model_path,model_file_all(iF).name));
