...
% 2015 04 18  Deal with rotation out-of-bound problem
%             (see folder 20151227_rotate_out_of_bnd)

clear
warning off
usrn = getenv('username');

% Add search path for functions
...
[~,script_name,~] = fileparts(mfilename('fullpath'));  % current running code

base_path = 'C:\Users\Wu-Jung Lee\Documents\fromDropbox\bp_processing';
...
bp_proc_path = 'proc_output_rousettus_checked';
bp_proc_file_all = dir(fullfile(base_path,bp_proc_path,'rousettus_20150825_36134*'));
data_fname = 'data_b36134.mat';

plot_opt_all_click = 1;     % plot rotated click summary figure
plot_opt_indiv_click = 0;   % plot click before/after rotation
save_opt = 0;               % save final output
freq_rot = 35e3;    % frequency used for ellipse fitting [Hz]

A.processed_files = bp_proc_file_all;
A.base_path = base_path;
A.data_path = bp_proc_path;
...
% Process all files
tic
raw_meas.az = cell(length(bp_proc_file_all),1);
raw_meas.el = cell(length(bp_proc_file_all),1);
raw_meas.call_dB = cell(length(bp_proc_file_all),1);
raw_meas.ch_include_idx = cell(length(bp_proc_file_all),1);
raw_meas.click_side = cell(length(bp_proc_file_all),1);
raw_meas.click_side_rep = cell(length(bp_proc_file_all),1);
raw_meas.click_num = cell(length(bp_proc_file_all),1);
rotate_data = cell(length(bp_proc_file_all),1);
shift_tilt_final.az = cell(length(bp_proc_file_all),1);
shift_tilt_final.el = cell(length(bp_proc_file_all),1);
 for iB = 1:length(bp_proc_file_all)

    bat_proc_file = bp_proc_file_all(iB).name;
    ss = strsplit(bat_proc_file,'_');
    save_fname = strjoin([script_name,ss(3:4)],'_');

    fprintf('File: %s\n',bat_proc_file);
    
    data = load(fullfile(base_path,bp_proc_path,bat_proc_file));
    ...
    for iC = good_call_idx'
        ...
        [click_side,raw,rot_max,rot_elpctr,rot_elpctr_tilt,fig_fit] = ...
            shift_rotate_bp(az(ch_include_idx),el(ch_include_idx),call_dB(ch_include_idx),'eckert4',plot_opt_indiv_click);
    ...
    if plot_opt_all_click
        figure(fig_clicks)
        saveSameSize(fig_clicks,'file',fullfile(save_path,['all_clicks_',save_fname]),'format','png','renderer','painters');
