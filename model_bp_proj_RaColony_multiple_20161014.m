% 2016 10 14  Generate multiple model_bp for use with Monte Carlo
%             simulation code 'model_bp_proj_MODELSHAPE_DATE'
...
bp_model_path = 'F:\Dropbox\Z_wjlee\projects\rousettus_bp\bp_bem_modeling\model_bp_save_multiple_20161014';
bp_model_file = dir(fullfile(bp_model_path,'*.mat'));
BP_all = cell(length(bp_model_file),1);
for iBP=1:length(bp_model_file)
    BP_indiv = load(fullfile(bp_model_path,bp_model_file(iBP).name));
    
    % Load model prediction
    idxnotnan = ~isnan(BP_indiv.pp_plot);
    [~,BP_indiv.vq_norm,BP_indiv.azq,BP_indiv.elq] = ...
        interp_bp(BP_indiv.az(idxnotnan)/180*pi,BP_indiv.el(idxnotnan)/180*pi,BP_indiv.pp_plot(idxnotnan),'rbf');
    BP_indiv.azq = BP_indiv.azq/pi*180;
    BP_indiv.elq = BP_indiv.elq/pi*180;
    [~,mm_idx] = max(BP_indiv.vq_norm(:));
    [BP_indiv.max_i,BP_indiv.max_j] = ind2sub(size(BP_indiv.vq_norm),mm_idx);
...
for iB = 1:length(rotate_data_file_all)
    ...
    % Select model bp
    bp_sel = unidrnd(length(bp_model_file));  % randomly select a BP from the pool
    BP = BP_all{bp_sel};
    
    S.model_bp_sel_idx = bp_sel;  % sequence index for the selected BP
    S.model_bp_file_sel = bp_model_file(bp_sel).name;  % filename of the selected BP
    ...
    [click_side,raw,rot_max,rot_elpctr,rot_elpctr_tilt] = ...
        shift_rotate_bp(az/180*pi,el/180*pi,vq_mic,'eckert4',it_shift_th,azel_bnd);
