...
simu_data_path = 'model_bp_proj_RaColony_multiple_20161014_std1.0';
...
num_ch = length(D.raw.az);

A.param.num_ch = num_ch;
A.param.map = D.map;

% Set save_fname
ss_save = strsplit(D.bp_model_file(1).name,'_');
model_freq_str = ss_save{6};

save_fname = sprintf('%s_%s_%s_bin%d_th%d',...
    script_name,noise_std,model_freq_str,binsize,threshold);
...
% Load model bp
bp_num = length(D.bp_model_file);
BP_all = cell(bp_num,1);
for iBP = 1:length(D.bp_model_file)
    BP_indiv = load(fullfile(D.bp_model_path,D.bp_model_file(iBP).name));
    ...
    [BP_indiv.xq,BP_indiv.yq] = mfwdtran(mstruct,BP_indiv.elq,BP_indiv.azq);
    BP_indiv.vq_norm(BP_indiv.vq_norm<min(cvec)) = min(cvec);
    
    ss = strsplit(D.bp_model_file(iBP).name,'_');
    ss = strjoin({ss{end-2:end}},'\\_');
    
    subplot(2,bp_num,iBP)    % model bp: left
    axesm(D.map.map_projection);
    contourf(-BP_indiv.xq,BP_indiv.yq,BP_indiv.vq_norm,cvec,'w');
    gridm('gcolor',cgrey,'glinestyle','-');
    framem('fedgecolor',cgrey);
    title(ss);
    tightmap
