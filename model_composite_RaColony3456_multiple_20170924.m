...
% 2017 10 05  -- Adapt code from `model_composite_RaColony3456_20170921`
%             -- This file is named for date 20170924 to correspond with the
%                date Monte Carlo simulation was done
...
simu_data_path = 'model_bp_proj_RaColony_multiple_multifreq_20170924_std1.0';
...
for iF=I
    ...
    fprintf('Plotting reconstructed bp at %d kHz\n',D.freq.all(iF)/1e3);
...
% Plot reconstructed bp for comparison
for iF = 1:num_freq

    fprintf('Plotting reconstructed bp at %d kHz',D.freq.all(iF)/1e3);

    fig_bp_cmp = figure('position',[200,60,1200,600]);

    % Reconstructed: left
    plot_bp_simple(subplot(211),interp_avg_left(iF).azq_avg, ...
                   interp_avg_left(iF).elq_avg, ...
    ...
    % Reconstructed: right
    plot_bp_simple(subplot(212),interp_avg_right(iF).azq_avg, ...
