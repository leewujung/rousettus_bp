...
% 2017 09 25  Same operation as in
%             `model_multifreq_indiv_bpctr_cntr_20170924` but with randomized model bp
...
data_path = 'model_bp_proj_RaColony_multiple_multifreq_20170924_std1.0';
            ...
            switch bpctr_opt
              ...
              case 'top'
                plotm(bpctr(iS,iF).top_el,bpctr(iS,iF).top_az,'.','markersize',8,...
                      'linewidth',2,'color',colorset(cnt,:));
              case 'ectr'
                if isempty(bpctr(iS,iF).ectr_el)
                    bpctr(iS,iF).ectr_el = NaN;
                    bpctr(iS,iF).ectr_az = NaN;
                end
