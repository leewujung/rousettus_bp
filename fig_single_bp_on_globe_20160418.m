...
if strcmp(usrn,'Wu-Jung')
    addpath('F:\Users\',usrn,'\Dropbox\0_CODE\MATLAB\rbfinterp_v1.2');
    addpath('F:\Users\',usrn,'\Dropbox\0_CODE\MATLAB\export_fig-master');
else
...
% call_num = 19;
interp_opt = 'rb_rbf';
save_root = 'C:\Users\Wu-Jung Lee\Documents\fromDropbox\bp_processing\analysis_results_figs';
[~,script_name,~] = fileparts(mfilename('fullpath'));
save_path = fullfile(save_root,script_name);
...
for iC=call_num
    ...
    ss_file = sprintf('b%s_t%s_c%02d_f%dkHz',ss{3},ss{4},iC,freq_wanted(iF)/1e3);
...
%     contour_vec = -3:-3:(floor(vq_norm_min/3)-1)*3;
    ...
    title(ss_title)
    set(gcf,'color','w');
    saveas(gcf,fullfile(save_path,[script_name,'_',ss_file,'.png']),'png');
    saveas(gcf,fullfile(save_path,[script_name,'_',ss_file,'.fig']),'fig');
    export_fig(fullfile(save_path,[script_name,'_',ss_file]),'-q101','-eps');  % high quality EPS
