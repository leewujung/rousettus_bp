...
data_path = 'proc_output_rousettus_checked';
...
% files = dir(fullfile(base_path,data_path,'*.mat'));
...
data = load(fullfile(base_path,data_path,data_file));
call_num = 20;
...
save_tf_name = sprintf('%s_%s_%s_c%02d_ch%02d',script_name,ss{3},ss{4},call_num,ch_num);
saveas(fig_tf,fullfile(save_path,save_tf_name),'fig');
saveSameSize(fig_tf,'file',...
    fullfile(save_path,save_tf_name),...
