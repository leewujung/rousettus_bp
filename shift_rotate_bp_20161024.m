function [click_side,raw,shift_max,shift_elpctr,shift_elpctr_tilt] = ...
        shift_rotate_bp_20161024(az,el,call_dB,map_proj,it_shift_th)
...
% 2016 07 25  update for the case when center of the best-fitting ellipse
%             for rot_max is outside of the projected globe
% 2016 10 25  Remove out-of-bound part of code
%             because rotating from az=180 to -180 is actually correct
%             same with el=90 to -90...
%             Need to re-interpolate vq for the new mic az/el locations
...
% Re-interpolate using new az,el locations   % added 2016/10/25
[~,vq_norm_max,azq_max,elq_max] =...
    interp_bp(az_max/180*pi,el_max/180*pi,call_dB,'rbf');
azq_max = azq_max/pi*180;
elq_max = elq_max/pi*180;
[xq_max,yq_max] = mfwdtran(mstruct,elq_max,azq_max);

shift_max.vq_norm = vq_norm_max;
...
M1.azq = azq_max;
M1.elq = elq_max;
M1.vq_norm = vq_norm_max;
M1.call_dB = call_dB;

% it_shift_th = 0.005;
while sqrt(E1.x0^2+E1.y0^2)>it_shift_th && n<5000
...
% Re-interpolate using new az,el locations   % added 2016/10/25
[~,vq_norm_ecen,azq_ecen,elq_ecen] =...
    interp_bp(shift_elpctr.az/180*pi,shift_elpctr.el/180*pi,call_dB,'rbf');
azq_ecen = azq_ecen/pi*180;
elq_ecen = elq_ecen/pi*180;
[xq_ecen,yq_ecen] = mfwdtran(mstruct,elq_ecen,azq_ecen);
[x_ecen,y_ecen] = mfwdtran(mstruct,shift_elpctr.el,shift_elpctr.az);

shift_max.vq_norm = vq_norm_ecen;
shift_max.azq = azq_ecen;
shift_max.elq = elq_ecen;
shift_max.x = x_ecen;
shift_max.y = y_ecen;
shift_max.xq = xq_ecen;
shift_max.yq = yq_ecen;
...
% Re-interpolate using new az,el locations   % added 2016/10/25
[~,vq_norm_re,azq_re,elq_re] =...
    interp_bp(az_ecen_tilt/180*pi,el_ecen_tilt/180*pi,call_dB,'rbf');
azq_re = azq_re/pi*180;
elq_re = elq_re/pi*180;
[xq_re,yq_re] = mfwdtran(mstruct,elq_re,azq_re);
[x_re,y_re] = mfwdtran(mstruct,el_ecen_tilt,az_ecen_tilt);

shift_elpctr_tilt.vq_norm = vq_norm_re;
shift_elpctr_tilt.azq = azq_re;
shift_elpctr_tilt.elq = elq_re;
shift_elpctr_tilt.x = x_re;
shift_elpctr_tilt.y = y_re;
shift_elpctr_tilt.xq = xq_re;
shift_elpctr_tilt.yq = yq_re;

% Fit ellipse
E_ecen_tilt = bp_fit_ellipse_azel(shift_elpctr_tilt,mstruct);  % check tilt compensation results
if isempty(E_ecen_tilt)
    shift_elpctr_tilt = [];
    return;
end
