...
% 2016 08 07  Update to use bat head model
...
model_path = 'model_bp_proj_bullethead_20160804b_std2.0';
...
script_name = [script_name,'_',model_calc_date];
save_path = fullfile(save_base_path,results_path,script_name);
if ~exist(save_path,'dir')
    mkdir(save_path);
end

title_text = sprintf('Beamwidth mean: %s %s, Noise: %s',model_calc_date,bw_opt,std);
...
idx_bad_data = elps_x_max<xy_lim(1) | elps_x_max>xy_lim(2) |...
...
idx_bad = idx_bad_data | idx_bad_model;

% Inverse map projection
[el_data,az_data] = minvtran(mstruct,elps_x_max(~idx_bad),elps_y_max(~idx_bad));
[el_model,az_model] = minvtran(mstruct,elps_x_max_model(~idx_bad),elps_y_max_model(~idx_bad));

A.idx_bad_data = idx_bad_data;
