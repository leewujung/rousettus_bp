function [vq,vq_norm,azq,elq] = interp_bp_res(az,el,mic_dB,method,res_deg)
...
% res_deg  interpolation resolution in degree
...
% Interpolation
maxref = max(mic_dB);
[azq,elq] = meshgrid(min(az):pi/180*res_deg:max(az),min(el):pi/180*res_deg:max(el));
