...
for iBP = 1:length(bp_model_file_all)
    ...
    [~,BP{iBP}.vq_norm,BP{iBP}.azq,BP{iBP}.elq] = ...
        interp_bp(BP{iBP}.az(idxnotnan)/180*pi,BP{iBP}.el(idxnotnan)/180*pi,BP{iBP}.pp_plot(idxnotnan),'rbf');
...
for iB = 1:length(rotate_data_file_all)
    ...
    if isempty(data.rot_elpctr)  % if data.rot_max.E.x0-y0 are not within
        ...
        % Select BP
        bp_sel = unidrnd(length(bp_model_file_all));  % randomly select a BP from the pool
        S.bp_model_file = bp_model_file_all(bp_sel);
        
        % Determine beam centr
        switch bpctr_opt
          case 'max'
            bpctr_data = meas.max;
            bpctr_model = BP{bp_sel}.max;
          case 'top'
            bpctr_data = meas.top;
            bpctr_model = BP{bp_sel}.top;
          case 'ectr'
            bpctr_data = meas.ectr;
            bpctr_model = BP{bp_sel}.ectr;
        end

        % Model bp
        if data.raw_meas.click_side==1  % right click --> no need to flip az
            az_model = BP{bp_sel}.az;
